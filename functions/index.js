...
exports.syncSavedataToFirestore = functions
  ...
  .https.onRequest(async (req, res) => {
    try {
      ...
      // Autenticação do Google Sheets via Runtime Config (legacy)
      ...
      // Lê valores A:C (Treinador | Dados | Senha) — Senha é ignorada
      ...
      // opcional: ?limit=10 pra testar
      const limit = Number(req.query.limit || 0);

      for (let i = 1; i < rows.length; i++) {
        if (limit && okCount >= limit) break;
        ...
        // "Dados" no sheet é JSON em string
        let dataObj;
        try {
          dataObj = JSON.parse(dataStr);
        } catch (e) {
          // se não parsear, salva como raw mesmo (pra não perder)
