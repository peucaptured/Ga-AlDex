...
def _build_assets_from_core_tilesets(tile_size_out: int) -> dict[str, Image.Image]:
    """
    Constrói o dicionário assets no MESMO formato esperado pelo render_map_png().
    - Lê: Assets/core_outdoor*.tsx + PNGs correspondentes
    - Recorta só o necessário (não carrega 8k tiles na memória sem motivo)
    - Cria variações via chaves __v00, __v01...
    """
    tsx_triplet = [
    ...
    if not all(os.path.exists(_resolve_asset_path_local(p)) for p in tsx_triplet):
        return {}

    # --- abre sheets ---
    parsed = [_parse_tsx(p) for p in tsx_triplet]
    sheets = []
    for meta, tsx_rel in zip(parsed, tsx_triplet):
    ...
    # pools
    floors = {"grass": [], "sand": [], "dirt": [], "rock": [], "snow": [], "cave": []}
    objs = {"tree": [], "bush": [], "flower": [], "rock_prop": [], "wall": [], "stalagmite": [], "peak": []}
    water_tiles = []  # para autotile: lista de dicts {img, cat, edges}

    # quotas (evita escanear o sheet inteiro)
    ...
    water_quota_total = 1200  # já dá shore bom por máscara

    def want_any_floor():
        return any(len(floors[k]) < floor_quota[k] for k in floors)
    ...
    for sheet, meta in [(sheets[0], parsed[0]), (sheets[1], parsed[1])]:
        ...
        for tid in range(count):
            # sai cedo se já pegou tudo
            if not (want_any_floor() or want_any_obj()):
                break
            ...
            # pisos sólidos (quase opacos)
            if cov > 0.92:
                if cat in floors and len(floors[cat]) < floor_quota[cat]:
                    floors[cat].append(tile)
                continue

            # objetos (transparência)
            if not want_any_obj():
                continue

            # heurística simples por cor/categoria
            stt = _tile_stats(tile)
            r, g, b = stt["avg"]
            bright = stt["bright"]

            if cat == "grass" and g > r + 15 and g > b + 10 and len(objs["tree"]) < obj_quota["tree"]:
                objs["tree"].append(tile)
            elif cat == "detail" and g > r + 10 and g > b + 5 and bright > 120 and len(objs["bush"]) < obj_quota["bush"]:
                objs["bush"].append(tile)
            elif (cat in ["detail", "grass"]) and bright > 140 and len(objs["flower"]) < obj_quota["flower"]:
                objs["flower"].append(tile)
            elif cat in ["rock", "cave"] and len(objs["rock_prop"]) < obj_quota["rock_prop"]:
                objs["rock_prop"].append(tile)
            elif cat == "cave" and bright < 120 and len(objs["stalagmite"]) < obj_quota["stalagmite"]:
                objs["stalagmite"].append(tile)
            elif cat == "rock" and bright < 130 and len(objs["peak"]) < obj_quota["peak"]:
                objs["peak"].append(tile)
            # paredes de dungeon (bem escuras e opacas-ish) — alguns têm borda
            elif cat == "cave" and cov > 0.55 and bright < 110 and len(objs["wall"]) < obj_quota["wall"]:
    ...
    # --------
    # 3) monta assets no formato do render atual
    # --------
    ...
    # água bordas por máscara (0..15)
    for m in range(16):
        variants = shore_by_mask.get(m, [])
        if not variants:
            continue
        # base (sem __v) = primeira variante
    ...
    # objetos fixos (o render atual escolhe por essas chaves)
