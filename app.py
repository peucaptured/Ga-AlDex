...
def render_compendium_page() -> None:
    ...
    # ---------------------------------------------------------
    # 3.1) TELA INICIAL (MENU EXCLUSIVO - BRANCO, ESTILO DARK SOULS)
    # ---------------------------------------------------------
    st.session_state.setdefault("compendium_home", True)

    if st.session_state.get("compendium_home", True):
        # CSS do menu inicial (tela branca, tipografia DS, cursor '>' e highlight forte)
        ds_font_family = "GaAL_DS" if font_css.strip() else "Cinzel"
        st.markdown(f"""
        <style>
            /* some header/footer */
            ...
            :root {{
                --home-font: {ds_font_family};
                --home-text: #0f0f0f;
                --home-dim: rgba(15,15,15,0.70);
                --home-line: rgba(15,15,15,0.18);
                --home-hi: rgba(15,15,15,0.10);
                --home-hi-strong: rgba(15,15,15,0.18);
            }}
            ...
            html, body, [class*="css"] {{
                font-family: var(--home-font), "Cinzel", "Times New Roman", serif !important;
                color: var(--home-text) !important;
            }}

            /* centraliza título (estilo tela inicial) */
            .gaal-home-title {{
                margin-top: 20vh;
                text-align: center;
                font-size: 56px;
            ...
            .gaal-home-press {{
                ...
                text-transform: uppercase;
                color: var(--home-dim);
                animation: gaal-blink 1.1s steps(2, end) infinite;
            }}
            @keyframes gaal-blink {{
                0% {{ opacity: 1; }}
                49% {{ opacity: 1; }}
                50% {{ opacity: 0; }}
                100% {{ opacity: 0; }}
            ...
            div[role="radiogroup"] > label:hover {{
                background: var(--home-hi) !important;
            ...
            /* cursor '>' + highlight mais forte no selecionado */
            div[role="radiogroup"] > label:has(input:checked) {{
                background: var(--home-hi-strong) !important;
                border-color: var(--home-line) !important;
            }}
        ...
        st.markdown("<div class='gaal-home-title'>BEM VINDO A GA'AL</div>", unsafe_allow_html=True)
        st.markdown("<div class='gaal-home-press'>PRESS ANY BUTTON</div>", unsafe_allow_html=True)

        # Menu (tabs) — ao selecionar, sai da tela inicial e abre o tomo correspondente.
        # Algumas versões antigas do Streamlit não aceitam index=None no radio; fazemos fallback.
        try:
            escolha = st.radio(
                "",
                ...
                horizontal=True,
                index=None,
                key="gaal_home_choice",
                label_visibility="collapsed",
            )
        except TypeError:
            # Fallback: mantém seleção padrão, mas exige um clique em "Iniciar" para prosseguir
            escolha = st.radio(
                "",
                ["Npcs", "Ginásios", "Locais"],
                horizontal=True,
            ...
            iniciar = st.button("Iniciar", key="gaal_home_start")
            if not iniciar:
                return

        if escolha:
            mapa = {"Npcs": "NPCs", "Ginásios": "Ginásios", "Locais": "Locais"}
            st.session_state["ds_tomo"] = mapa.get(escolha, "Locais")
            st.session_state["compendium_home"] = False
            st.rerun()
