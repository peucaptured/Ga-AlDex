...
class BiomeGenerator:
    ...
    def __init__(self, assets_root: str | Path = DEFAULT_ASSETS_ROOT):
        ...
        # Triagem do pack "foliage": separar itens que realmente parecem árvores
        # (silhueta alta / multi-tile) dos demais (arbustos/plantas). Isso permite:
        # - Floresta: árvores do foliage aparecem com boa frequência
        # - Biomas com grama: árvores do foliage aparecem com baixa frequência
        self.foliage_trees, self.foliage_plants = self._classify_sprite_pack(self.foliage_sprites)
    ...
    def _classify_sprite_pack(self, sprites: List[Sprite]) -> Tuple[List[Sprite], List[Sprite]]:
        """Heurística simples para separar árvores de plantas/arbustos.

        Critérios:
        - sprites multi-tile altos (>=2 tiles) tendem a ser árvores
        - bbox alpha alto e mais "vertical" também tende a árvore
        """
        trees: List[Sprite] = []
        plants: List[Sprite] = []
        for sp in sprites:
            ...
            x0, y0, x1, y1 = alpha_bbox(im)
            bw, bh = (x1 - x0), (y1 - y0)
            # normaliza para "tiles"
            tiles_h = max(1.0, bh / float(self.tile_raw_px))
            tiles_w = max(1.0, bw / float(self.tile_raw_px))

            # árvore: altura >= ~1.6 tiles OU multi-tile vertical, e não exageradamente larga
            is_tree = (
                sp.tiles_h >= 2
                or (tiles_h >= 1.6 and tiles_h >= tiles_w * 0.85)
                or (bh > bw * 1.05 and bh >= self.tile_raw_px * 1.25)
            )
...
# --- Transitions / special terrains ---
            ...
            for x in range(grid_w):
                ...
                if t == 5:
                    ...
                    def _pick_water_core_prefer(prefer: str, rng) -> str:
                        ...
                        """
                        # Lista "plain" padrão do water_core (TileSet.plain)
                        plain_lst = getattr(self.water_core, "plain", None) or []
                        ...
                        # Se não houver listas separadas, tenta filtrar a plain pelo nome do arquivo
        ...
        # Forest: muitas árvores + arbustos. Também usa árvores vindas do pack "foliage".
        if biome == "forest":
            self._place_sprites(canvas, rng, self.forest_trees, occ, tile_px, attempts=6500, density=0.18, allowed_anchor=is_grass)
            self._place_sprites(canvas, rng, self.forest_shrubs, occ, tile_px, attempts=4200, density=0.09, allowed_anchor=is_grass)
            # árvores adicionais (foliage) com boa frequência
            self._place_sprites(canvas, rng, self.foliage_trees, occ, tile_px, attempts=3200, density=0.06, allowed_anchor=is_grass)
            # plantas/arbustos do foliage pra preencher vazios
            self._place_sprites(canvas, rng, self.foliage_plants, occ, tile_px, attempts=2600, density=0.03, allowed_anchor=is_grass)

        # Prairie: flores + plantas; poucas árvores (bem menos frequente)
        if biome == "prairie":
            self._place_sprites(canvas, rng, self.flower_sprites, occ, tile_px, attempts=4800, density=0.09, allowed_anchor=is_grass)
            self._place_sprites(canvas, rng, self.foliage_plants, occ, tile_px, attempts=3600, density=0.055, allowed_anchor=is_grass)
            # árvores raras em grama
            self._place_sprites(canvas, rng, self.foliage_trees, occ, tile_px, attempts=900, density=0.006, allowed_anchor=is_grass)
            # chance pequena de arbusto da floresta
            self._place_sprites(canvas, rng, self.forest_shrubs, occ, tile_px, attempts=1400, density=0.012, allowed_anchor=is_grass)

        # Dirt: vegetação bem mais esparsa
        if biome == "dirt":
            self._place_sprites(canvas, rng, self.foliage_plants, occ, tile_px, attempts=2200, density=0.02, allowed_anchor=is_grass)
            self._place_sprites(canvas, rng, self.foliage_trees, occ, tile_px, attempts=600, density=0.003, allowed_anchor=is_grass)
