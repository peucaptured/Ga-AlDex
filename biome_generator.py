{
type: uploaded file
fileName: Essencial/biome_generator.py
fullContent:
...
- Cave updates: Remove lighting/shadows, use cave_overlay, fit assets to tiles.
...
class AtlasTileSet:
    ...
    def __init__(self, folder: Path, json_name: str):
        ...
        if not json_path.exists():
            # Fallback silencioso para evitar crash se o arquivo não existir
            self.atlas = Image.new("RGBA", (1, 1))
            return
    ...
    def pick_any(self, rng: random.Random) -> Optional[AtlasTile]:
        if not self.tiles:
            return None
...
class BiomeGenerator:
    ...
    def __init__(self, assets_root: str | Path = DEFAULT_ASSETS_ROOT):
        ...
        sample_path = self.root / "water_grass"
        if sample_path.exists():
            try:
                sample = next(iter(sample_path.glob("water_grass_m00_v0.png")), None)
            except:
                sample = None
        else:
            sample = None
        ...
        self.cave_sprites  = load_sprite_entries(self.root / "cave_overlay" / "cave_overlay.json", self.root / "cave_overlay", self.tile_raw_px)
    ...
    def _load_sprite_with_shadow(self, p: Path, tile_px: int, add_shadow: bool = True) -> Optional[Image.Image]:
        key = (str(p), tile_px, "shadow" if add_shadow else "no_shadow")
        if key in self._img_cache:
            return self._img_cache[key]  # may be None
    ...
    def _place_sprites(
        ...
        for _ in range(attempts):
            ...
            # Se forçar caber na tile, assumimos dimensão 1x1 na grid lógica
            if force_fit_tile:
                tw, th = 1, 1
            else:
            ...
            im = self._load_sprite_with_shadow(sp.path, tile_px, add_shadow=add_shadow)
            if im is None:
            ...
            if force_fit_tile:
                # Redimensiona para caber estritamente dentro de tile_px x tile_px
                # Mantendo aspect ratio (thumbnail) ou distorcendo? "Caber dentro" geralmente preserva aspecto.
                # Usaremos thumbnail para garantir que caiba.
                im.thumbnail((tile_px, tile_px), Image.Resampling.LANCZOS)
                
                # Centraliza no tile
                # O sprite original pode ter pivot, mas se estamos forçando 1x1, melhor centralizar
                # visualmente no tile (base do tile)
                
                # Se a imagem ficou menor que o tile (ex: era comprida), centraliza horizontalmente
                off_x = (tile_px - im.size[0]) // 2
                off_y = (tile_px - im.size[1]) # Alinha no fundo do tile
                
                tgt_x = x * tile_px + off_x
                tgt_y = y * tile_px + off_y
...
# ---------- public API ----------
    def generate(self, biome: str, grid_w: int = 32, grid_h: int = 32, tile_px: int = 64, seed: int = 0) -> Image.Image:
        ...
        # --- Fill Background if Cave (to fix "not rendering all ground") ---
        if biome == "cave":
            # Preenche tudo com terra leve primeiro para garantir que não haja buracos
            for y in range(grid_h):
                for x in range(grid_w):
        ...
        for y in range(grid_h):
            for x in range(grid_w):
                ...
                # Se for caverna, já preenchemos o background, então desenhamos apenas se for "rock" (4)
                # ou se quisermos sobrepor. Mas como já preenchemos com light_dirt (2),
                # só precisamos desenhar se t == 4 E tivermos tiles específicos de rock_floor.
                if biome == "cave":
                    if t == 4:
                        # Tenta desenhar piso de rocha se disponível
                        ...
                        # Se não tiver rock_floor, já está preenchido com light_dirt (o fallback anterior)
                    # Se t == 2, já está preenchido.
                    continue
                ...
                # t=4 fora da caverna (raro, mas fallback)
                elif t == 4: 
                    if getattr(self, "rock_floor", None) is not None and getattr(self.rock_floor, "plain", None):
                        tilep = self.rock_floor.pick_plain(rng)
                        blit_tile(x, y, self._load_resized(tilep, tile_px, force_tile=True))
...
# --- Transitions / special terrains ---
        ...
        # EXCETO CAVERNA: Caverna tem lógica própria de rochas agora
        if biome != "cave":
        ...
        if biome == "cave":
            # 1. Filtra assets para respeitar "no máximo tamanho 2x2"
            small_cave_sprites = [
                s for s in self.cave_sprites 
                if s.tiles_w <= 2 and s.tiles_h <= 2
            ]
            
            # 2. Renderiza rochas usando cave_overlay
            # - Sem iluminação/sombra (add_shadow=False)
            # - Redimensionado para caber na tile (force_fit_tile=True)
            self._place_sprites(
                canvas, 
                rng, 
                small_cave_sprites, 
                ...
                add_shadow=False,     # Retira iluminação
                force_fit_tile=True   # Redimensiona para caber
            )
            
            # Misc extra se sobrar espaço
            self._place_sprites(canvas, rng, self.misc_sprites, occ, tile_px, attempts=1000, density=0.01, allowed_anchor=is_land)
