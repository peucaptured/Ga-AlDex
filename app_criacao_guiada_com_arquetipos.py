...
def _viab_suggest_moves(db: "MoveDB", archetype: dict) -> tuple[list["Move"], list[str]]:
    ...
    # fallback final: varrer no texto completo
...
def _viab_apply_to_session(archetype: dict, rank_default: int, excel_path: str) -> tuple[list[str], list[str], str | None]:
    ...
    moves_found, missing = _viab_suggest_moves(db, archetype)
    ...
    if st.session_state["cg_view"] == "guided":
        ...
        if pname:
                ...
                if viab_text and viab_text.strip() and viab_text.strip().lower() not in {"nan", "sem dados.", "sem dados"}:
                    with st.expander("📌 Arquétipo de Viabilidade (1 clique)", expanded=True):
                        ...
                        if not archetypes:
                            ...
                            with colA:
                                if st.button("✅ Aplicar arquétipo (adicionar golpes + setar habilidade)", key="cg_viab_apply_btn"):
                                    added, missing, abil = _viab_apply_to_session(arch, int(rank_default), excel_path_viab)
            ...
            with tabs[3]:
                ...
                with sub_tabs[1]:
                    st.caption(
                        "Crie um poder customizado respondendo perguntas simples. "
                        "A build final é gerada pelo MESMO interpretador que você já usa (sem mudar lógica)."
                    )
                    ...
                    if interpret_effects_to_build is None:
                        ...
                        if submit:
                            ...
                            # estima PP usando o mesmo método do Move (sem mudar lógica)
                            tmp_mv = Move(
                                ...
                                descricao="Criado via Assistente",
                                build=build,
                                how_it_works="",
                                resist_stat="",
                                ranged=bool(ranged),
